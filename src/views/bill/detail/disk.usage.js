...
export default function DiskDetail(props) {
    ...
    useEffect(() => {
        if (props && props.date && props.accountId) {
            ...
            getUsageApi({
                url: "/bill/stats-detail?accountId=" + props.accountId + "&type=disk&period=" + startDate
            });
        }
        ...
        <MaterialTable
            ...
            columns={[
                ...
                },
                {
                    ...
                    type: "numeric",
                    filtering: false,
                    render: row => <span>{row.number.toFixed(2)}</span>
                }
            ]}
            data={data}
            options={{
