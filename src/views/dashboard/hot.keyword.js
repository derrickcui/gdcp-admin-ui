...
import {
    DATAMAP_SUCCESS,DATAMAP_KEYWORD
}  from '../../constant/index';
...
export default function HotKeyword() {
    ...
    useEffect(() => {
        if (getHotkeyWordRankLoading === false) {
            if (getHotkeyWordRankError) {
                setMessage({
                    type: MESSAGE_ERROR,
                    text: getRowText("getHotkeyWordRankError")
                });
    ...
    return(
        <Card variant="outlined" className={classes.root}>
            <CardHeader title={<span style={{fontSize: 14}}>{getRowText("hotKeyWord")}</span>} className={headerClasses.header}/>
            <CardContent>
                <MaterialTable
                    components={{
